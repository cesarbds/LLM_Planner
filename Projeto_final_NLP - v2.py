...
places_1 = ["dinnertable", "smalltable", "bed", "rack", "kitchen_drawer", "countertop"]
...
client       = Groq(api_key='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')
GROQ_API_KEY = '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'
...
places = {
   ...
   "kitnet" : [3.7, 4.95, 0.8905],
   "kitnet2" : [3.4, 4.97, 0.8431],
    ...
    if item and file_path == 'objects_tmp.json':
      if item['resources']['pickeable']:
        ...
        if target in objects_close:
          ...
          res, object_handle = sim.simxGetObjectHandle(clientID, "/"+target.title(), sim.simx_opmode_blocking)
          ret, robotHandle = sim.simxGetObjectHandle(clientID, "/P3DX", sim.simx_opmode_blocking)
          res = sim.simxSetObjectParent(clientID, object_handle, robotHandle, True ,sim.simx_opmode_blocking)
          res, pos = sim.simxGetObjectPosition(clientID, robotHandle, -1, sim.simx_opmode_blocking)
      ...
      if item['resources']['placeable']: 
        if place in objects_close:
          if place_item['resources']['surface']:
            #print(target.title())
            res, obj = sim.simxGetObjectHandle(clientID, target.title(), sim.simx_opmode_blocking)
            ...
            if "P3DX" in retStrings:
              res, collision_state, retFloats, objects_close, retBuffer = sim.simxCallScriptFunction(clientID,'/P3DX/objects_detector',sim.sim_scripttype_childscript,'getCollisionObjects',[], [],[],bytearray(),sim.simx_opmode_blocking )
